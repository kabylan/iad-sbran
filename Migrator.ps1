...
Switch ($action)
{
   ...
   'update' { cmd.exe /c "dotnet ef database update $migrationName -p $projectPath --startup-project $startupProject --connection ""User ID=scientist;Password=P@ssw0rd;Host=localhost;Port=5432;Database=IadSbran"" --context DomainContext && dotnet ef database update $migrationName -p $projectPath --startup-project $startupProject --context SystemContext"}
}